    ...
    tabFormat: {
		"Main":{
			content:[
				...
				if (player.ep.buyables[11].gte(3)) table += (player.mp.buyables[21].gte(1)?'<br><span class="ef">3rd effect: Hyper Boost effect base +' + format(tmp.ep.threeEffect,4)+"</span":'<br>3rd effect: Hyper Boost effect base +' + format(tmp.ep.threeEffect,4))
				if (player.ep.buyables[11].gte(4)) table += (player.mp.buyables[21].gte(2)?'<br><span class="ef">4th effect: Transcend Points hardcap starts ' + format(tmp.ep.fourEffect,4) + "x later</span>":'<br>4th effect: Transcend Points hardcap starts ' + format(tmp.ep.fourEffect,4) + "x later")
				if (player.ep.buyables[11].gte(5)) table += (player.mp.buyables[21].gte(3)?'<br><span class="ef">5th effect: Add an Hyper-Prestige Points inflation (^' + format(tmp.ep.fiveEffect.eff,4) + " to gain), that starts at "+ format(tmp.ep.fiveEffect.start,4) + " Hyper-Prestige Points</span>":'<br>5th effect: Add an Hyper-Prestige Points inflation (^' + format(tmp.ep.fiveEffect.eff,4) + " to gain), that starts at "+ format(tmp.ep.fiveEffect.start,4) + " Hyper-Prestige Points")
				if (player.ep.buyables[11].gte(6)) table += (player.mp.buyables[21].gte(4)?'<br><span class="ef">6th effect: Transcend Points gain in Transcend Challenges is x' + format(tmp.ep.sixEffect,4) + " better.</span>":'<br>6th effect: Transcend Points gain in Transcend Challenges is x' + format(tmp.ep.sixEffect,4) + " better.")
				if (player.ep.buyables[11].gte(7)) table += (player.mp.buyables[21].gte(5)?`<br><span class="ef">7th effect: Softcap of Prestige Boost's effect starts +` + format(tmp.ep.sevenEffect,4) + " later.</span>":"<br>7th effect: Softcap of Prestige Boost's effect starts +" + format(tmp.ep.sevenEffect,4) + " later.")
				if (player.ep.buyables[11].gte(8)) table += (player.mp.buyables[21].gte(6)?`<br><span class="ef">8th effect: Add +` + format(tmp.ep.eightEffect,4) + " to prestige energy gain exponent.</span>":"<br>8th effect: Add +" + format(tmp.ep.eightEffect,4) + " to prestige energy gain exponent.")
				return table}],
				"buyables",
                "upgrades"
			]
