...
// data = a function returning the content (actually HTML)
	...
	Vue.component('challenge', {
		...
		<div v-if="tmp[layer].challenges && tmp[layer].challenges[data]!== undefined && tmp[layer].challenges[data].unlocked && !(options.hideChallenges && maxedChallenge(layer, [data]) && !inChallenge(layer, [data]))"
			...
			<button v-bind:class="{ longUpg: true, can: true, [layer]: true }" v-bind:style="tmp[layer].challenges[data].buttonStyle?tmp[layer].challenges[data].buttonStyle : {'background-color': tmp[layer].color}" v-on:click="startChallenge(layer, data)">{{challengeButtonText(layer, data)}}</button><br><br>
    ...
    Vue.component('upgrade', {
		...
		<div v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!==undefined">
			...
			<span class='upgNum'>{{data}}</span>
				...
				<br><span v-if="tmp[layer].upgrades[data].costDescription" v-html="tmp[layer].upgrades[data].costDescription"></span>
				<span v-if="!tmp[layer].upgrades[data].costDescription">Cost: {{ formatWhole(tmp[layer].upgrades[data].cost) }} <span v-html="tmp[layer].upgrades[data].currencyDisplayName ? tmp[layer].upgrades[data].currencyDisplayName : tmp[layer].resource"></span></span>
			</span>
	...
	Vue.component('milestones', {
		...
		<div v-if="tmp[layer].milestones">
			<table>
				<tr v-for="id in (data === undefined ? options.reverseMilestones==true?Object.keys(tmp[layer].milestones).reverse() : Object.keys(tmp[layer].milestones) : data)" v-if="tmp[layer].milestones[id]!== undefined && tmp[layer].milestones[id].unlocked && milestoneShown(layer, id)">
	...
	Vue.component('buyables', {
		...
		<div v-if="tmp[layer].buyables" class="upgTable">
			...
			<div v-for="row in (data === undefined ? tmp[layer].buyables.rows : data)" class="upgRow">
				<div v-for="col in tmp[layer].buyables.cols"><div v-if="tmp[layer].buyables[row*10+col]!== undefined && tmp[layer].buyables[row*10+col].unlocked" v-bind:style="{  'height': (data ? data : 'inherit'),}">
	...
	Vue.component('gridable', {
		...
		v-on:click="clickGrid(layer, data)"  @mousedown="start" @mouseleave="stop" @mouseup="stop" @touchstart="start" @touchend="stop" @touchcancel="stop"></span>
		<node-mark :layer='layer' :data="gridRun(this.layer, 'getMark', player[this.layer].grid[this.data], this.data)" :offset="10" :scale="1.2"></node-mark>
	...
	Vue.component('achievement', {
		...
		<div v-if="tmp[layer].achievements && tmp[layer].achievements[data]!== undefined && tmp[layer].achievements[data].unlocked" v-bind:class="{ [layer]: true, achievement: true, tooltipBox:true, locked: !hasAchievement(layer, data), bought: hasAchievement(layer, data)}"
			...
			<tooltip v-bind:style="tmp[layer].achievements.tooltipStyle?tmp[layer].achievements.tooltipStyle:{}":text="
		...
		"></tooltip>
			...
			<span class='achNum'>{{Math.floor(data/10)<=1?data-10:data-11}}</span>
			</div>
		`
	})
